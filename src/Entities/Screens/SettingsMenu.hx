...
class SettingsMenu extends Entity
{
    ...
    public function new()
    {
        ...
        graphic = Image.createRect(HXP.width, HXP.height, 0x000000, .5);

        menuWidth = Math.floor(HXP.width*.7);
        menuHeight = Math.floor(HXP.height *.6);
        var submenuBackground:Graphic = addGraphic(Image.createRect(menuWidth+6, menuHeight+6, 0x444444));
        submenuBackground.x = (HXP.halfWidth) - (menuWidth+6)/2;
        submenuBackground.y = (HXP.halfHeight) - (menuHeight+6)/2;

        submenu = addGraphic(Image.createRect(menuWidth, menuHeight, 0x555555));
        submenu.x = (HXP.halfWidth) - menuWidth/2;
        submenu.y = (HXP.halfHeight) - menuHeight/2;

        muteBackgroundWidth = 52;
        muteBackgroundHeight = 52;
